...
abstract class Terms_Controller extends Rest_Base {
				...
				array(
					...
					'args'                => array_merge(
						...
						array(
							'name' => array(
								'type'        => 'string',
								'description' => __( 'Name for the resource.', 'wp-oxynate' ),
								'required'    => true,
							),
						)
					),
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_view', __( 'Sorry, you cannot list resources.', 'wp-oxynate' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'wp-oxynate' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_view', __( 'Sorry, you cannot view this resource.', 'wp-oxynate' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_edit', __( 'Sorry, you are not allowed to edit this resource.', 'wp-oxynate' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_delete', __( 'Sorry, you are not allowed to delete this resource.', 'wp-oxynate' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( ! $taxonomy || ! taxonomy_exists( $taxonomy ) ) {
			return new WP_Error( 'wp_oxynate_rest_taxonomy_invalid', __( 'Taxonomy does not exist.', 'wp-oxynate' ), array( 'status' => 404 ) );
		}
		...
		if ( $id ) {
			...
			if ( is_wp_error( $term ) || ! $term ) {
				return new WP_Error( 'wp_oxynate_rest_term_invalid', __( 'Resource does not exist.', 'wp-oxynate' ), array( 'status' => 404 ) );
			}

			return $this->check_post_term_permissions( $taxonomy, $context, $term->term_id );
		}

		return $this->check_post_term_permissions( $taxonomy, $context );
	}
	...
	public function get_items( $request ) {
		 ...
		 */
		$prepared_args = apply_filters( "wp_oxynate_rest_{$taxonomy}_query", $prepared_args, $request );
	...
	public function create_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $taxonomy ) ) {
				return new WP_Error( 'wp_oxynate_rest_taxonomy_not_hierarchical', __( 'Can not set resource parent, taxonomy is not hierarchical.', 'wp-oxynate' ), array( 'status' => 400 ) );
			}
		 ...
		 */
		do_action( "wp_oxynate_rest_insert_{$taxonomy}", $term, $request, true );
	...
	public function update_item( $request ) {
		 ...
		 * @param boolean         $creating  True when creating term, false when updating.
		 */
		do_action( "wp_oxynate_rest_insert_{$taxonomy}", $term, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $retval ) {
			return new WP_Error( 'wp_oxynate_rest_cannot_delete', __( 'The resource cannot be deleted.', 'wp-oxynate' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( "wp_oxynate_rest_delete_{$taxonomy}", $term, $response, $request );
	...
	public function get_collection_params() {
		...
		$params['offset']     = array(
			'description'       => __( 'Offset the result set by a specific number of items. Applies to hierarchical taxonomies only.', 'wp-oxynate' ),
			'type'              => 'integer',
		...
		$params['orderby']    = array(
			'description'       => __( 'Sort collection by resource attribute.', 'wp-oxynate' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_key',
			'default'           => 'name',
			'enum'              => array(
		...
		$params['hide_empty'] = array(
			'description'       => __( 'Whether to hide resources not assigned to any posts.', 'wp-oxynate' ),
			'type'              => 'boolean',
			'default'           => false,
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['parent']     = array(
			'description'       => __( 'Limit result set to resources assigned to a specific parent. Applies to hierarchical taxonomies only.', 'wp-oxynate' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['post']    = array(
			'description'       => __( 'Limit result set to resources assigned to a specific post.', 'wp-oxynate' ),
			'type'              => 'integer',
			'default'           => null,
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['slug']       = array(
			'description'       => __( 'Limit result set to resources with a specific slug.', 'wp-oxynate' ),
			'type'              => 'string',
			'validate_callback' => 'rest_validate_request_arg',
		);
    ...
    /**
     * Check permissions of post terms on REST API.
     *
     ...
     * @return bool
     */
    function check_post_term_permissions( $taxonomy, $context = 'read', $object_id = 0 ) {
        $contexts = array(
            'read'   => 'manage_terms',
            'create' => 'edit_terms',
            'edit'   => 'edit_terms',
        ...
        return apply_filters( 'wp_oxynate_rest_check_permissions', $permission, $context, $object_id, $taxonomy );
    }
